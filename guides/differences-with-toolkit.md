...
- Minimal reexports from submodules. `@actions/core` exports `core.summary` from
  `@actions/core/summary`. Pontil does not. To work with the action summary, use
  `pontil/summary`. Types _may_ be reexported from `pontil/types`, but the
  documentation remains there. (The `pontil/types` module exists to prevent
  circular dependencies with internal modules while keeping the documentation
  public.)
...
- Type variant lists are preferred over records for options where most of the
  ...
  get_input_opts(name, [InputRequired, TrimInput])
  ```
