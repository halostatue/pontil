...
pub fn get_id_token(
  ...
  use id_token_url <- utils.try_promise(get_id_token_url())
...
fn call(
  ...
  use req <- utils.try_promise(create_request(id_token_url, audience))
...
fn get_request_token() -> Result(String, PontilError) {
  case envoy.get("ACTIONS_ID_TOKEN_REQUEST_TOKEN") {
    Ok(token) -> Ok(token)
    _error -> missing_env_var("ACTIONS_ID_TOKEN_REQUEST_TOKEN")
  }
}
...
fn get_id_token_url() -> Result(String, PontilError) {
  case envoy.get("ACTIONS_ID_TOKEN_REQUEST_URL") {
    Ok(url) -> Ok(url)
    _error -> missing_env_var("ACTIONS_ID_TOKEN_REQUEST_URL")
  }
}
